...
public class AkvKeyStore : IKeyStore
{
        ...
        Dictionary<string, string> tags)
    {
        ...
        try
        {
            ...
            SnpCACIAttestationReport report = encKeyAndReport.Report.SnpCaci!;
        ...
        Dictionary<string, string> tags)
    {
        ...
        {
            var attestationJson = await File.ReadAllTextAsync(
                "/app/insecure-virtual/signing/attestation.json");
            signingKeyAndReport =
        ...
        try
        {
            ...
            SnpCACIAttestationReport report = signingKeyAndReport.Report.SnpCaci!;
            var snpReport = SnpReport.VerifySnpAttestation(
                report.Attestation,
                report.PlatformCertificates,
                report.UvmEndorsements);
    ...
    {
        return Attestation.ToReportDataHashValue(Encoding.UTF8.GetBytes(publicKey)).ToLower();
    }
    ...
    private async Task VerifyKeyImportedByService(string publicKey, SnpCACIAttestationReport report)
    {
        SnpReport snpReport;
        try
        {
        ...
        string publicKey)
    {
        ...
        await this.VerifyKeyImportedByService(publicKey, attestationReport.SnpCaci!);
